...
**Superficie AIDL `IDVRService`** (⚠️ correzione 2026-06-25: NON tutti i metodi sono wrappati in `DvrHelper` — verificato leggendo `DvrHelper.java` completo):
- **Wrappati in `DvrHelper`** (usabili subito): `getSettingInfo()` / `setSettingInfo(SettingInfo)` (leggi/scrivi impostazioni DVR), `startPreviewByChannel(channel, surface)`, `setPlaybackSurface(surface)`, `dvrControlCmd(cmd, arg0, arg1)`, `getDvrState`, `getFileInfos`/`operationFileList`/`refreshFileInfo` (gestione file registrati), `enterSource`.
- **NON wrappati** (serve aggiungere il wrapper in `DvrHelper` per chiamarli): `setCameraDeviceParam(csi, channel, type, value)` / `getCameraDeviceParam(csi, channel, type)` (parametri a livello camera), `sendSettingCommand(msg, arg0, arg1)`. (Anche tutta la superficie AVM/retro/radar/car-model-pay e NON wrappata — vedi [[ntg062-architecture-ipc]].)
