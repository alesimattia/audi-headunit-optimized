...
L'utente non ha root né chiave → si usa la **Strada C**. ✅ **CONFERMATO sulla testata dell'utente (2026-06-25)**: sotto Strada C (app come pacchetto separato, **senza privilegi system**) la **telemetria `CarInfo` legge dati reali** — porte e velocità funzionano — e la pagina Sistema gira. Quindi l'AIDL `com.spd.carinfo` **si collega anche senza UID system**: l'incertezza residua è solo SE un singolo `what` venga popolato dal CAN-box per l'Audi, NON se il servizio si collega. Restano da verificare singolarmente solo `SpdManager` (luminosità/audio focus) e la scrittura `Settings.System`. Prima di installare la versione modificata va **disinstallato l'originale** (firma diversa). Dettagli build/firma in [[ntg062-modding-build]]; stato modifiche in [[ntg062-applied-mods]].
