...
public abstract class MediaPlaylistAgent {
    ...
    public final void notifyPlaylistChanged() {
        ...
        for (int i = 0; i < callbacks.size(); i++) {
            ...
            executor.execute(new Runnable() { // from class: androidx.media.MediaPlaylistAgent.1
    ...
    public final void notifyPlaylistMetadataChanged() {
        ...
        for (int i = 0; i < callbacks.size(); i++) {
            final PlaylistEventCallback callback = callbacks.keyAt(i);
            Executor executor = callbacks.valueAt(i);
            executor.execute(new Runnable() { // from class: androidx.media.MediaPlaylistAgent.2
                @Override // java.lang.Runnable
                public void run() {
    ...
    public final void notifyShuffleModeChanged() {
        ...
        for (int i = 0; i < callbacks.size(); i++) {
            final PlaylistEventCallback callback = callbacks.keyAt(i);
            Executor executor = callbacks.valueAt(i);
            executor.execute(new Runnable() { // from class: androidx.media.MediaPlaylistAgent.3
                @Override // java.lang.Runnable
                public void run() {
    ...
    public final void notifyRepeatModeChanged() {
        ...
        for (int i = 0; i < callbacks.size(); i++) {
            final PlaylistEventCallback callback = callbacks.keyAt(i);
            Executor executor = callbacks.valueAt(i);
            executor.execute(new Runnable() { // from class: androidx.media.MediaPlaylistAgent.4
                @Override // java.lang.Runnable
                public void run() {
